...
- 結果表示は「ユーザー CSV 未ドロップ時は example/pentacene/ のサンプル（大野さん提供、後日受領）を表示、ドロップで切替」方式
...
- 大野さんの既存スクリプト**受領済み**（2026-07-04, `legacy/ono_scripts/`）:
  - `stepwise_optimization/` — Step 1〜3 の山登り法最適化（make_*.py が gjf/ジョブスクリプト生成、step*.py が制御・結果集計）
  - `tcal_csv/` — transfer integral 一括計算（松井研 tcal ベース、https://github.com/matsui-lab-yamagata/tcal）
  - これらを src/csp/ へ統合するのが次の実装タスク
