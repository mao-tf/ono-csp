...
- [ ] tcal で pbepbe を使っていた経緯の確認（2026-07-06 三好さんの指摘で経路を精査）。
      ...
      `tcal_csv.py`の`pre_process()`が`#pbepbe/6-311G**`を直書きして生成したもの。
      つまり**コード上、自動パイプラインは確実にpbepbeを使う設計**（B3LYPのデフォルトは
      この経路では使われない）。論文Fig.11の実際の計算で`tcal_csv.py`をそのまま使ったのか、
      `tcal_1.py -x`を手動実行したのかは大野さんに確認しないと分からない。
      pbepbe由来なら2026-07-05のb3lyp/6-31g*修正を戻す必要がある
