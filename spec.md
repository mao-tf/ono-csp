...
2. 上記修正後も「黒い四角以外をクリックすると画面はリロードされるが3Dモデルが
   更新されない」不具合が発覚。原因は`px.imshow(pivot, ...)`（DataFrameを直接渡す形）だと、
   軸の目盛りラベルには実際のRa/Rb値を表示するが、**クリックイベントが返す`x,y`は
   ピクセルインデックス（0,1,2…）**になっており、実データ値と一致しなかったため
   （散布図側は実データ値をそのまま返すので黒四角クリックだけは正しく動いていた）。
   → `px.imshow(pivot.values, x=pivot.columns, y=pivot.index, ...)`と**明示的に座標配列を渡す**
   ことで解決。トレースのx/yが実際のRa/Rb値になることをスクリプトで検証済み
