...
- [ ] tcal で pbepbe を使っていた経緯の確認（2026-07-06 三好さんの指摘で経路を精査。
      **注意**: `legacy/ono_scripts/tcal_csv/tcal_csv.py`は現在`b3lyp/6-31g*`表記だが、
      これは2026-07-05に**こちらで書き換えた後の状態**。大野さんが元々アップロードした
      コード（コミット`60a4f6e`）では`#pbepbe/6-311G**`だった）。
      `tcal_1.py`自体は`convert_xyz_to_gjf()`のデフォルトが`B3LYP/6-31G(d,p)`（論文と一致）だが、
      これは`-x`フラグ指定時のみ呼ばれる関数。`tcal_csv.py`の実際の自動パイプラインは
      `tcal_1.py`を`-r`（読み取り専用、Gaussian未実行）で呼んでおり、`.gjf`自体は
      `tcal_csv.py`の`pre_process()`が（元は）`#pbepbe/6-311G**`を直書きして生成したもの。
      つまり**受領時点のコード上、自動パイプラインは確実にpbepbeを使う設計**だった
      （B3LYPのデフォルトはこの経路では使われない）。論文Fig.11の実際の計算で
      `tcal_csv.py`をそのまま（pbepbeで）使ったのか、`tcal_1.py -x`を手動実行（b3lyp）
      したのかは大野さんに確認しないと分からない。pbepbe由来と分かれば
      2026-07-05のb3lyp/6-31g*修正を元に戻す必要がある
...
- [x] ~~受領コード一式にN形を発見・最適化するロジックが見当たらない件~~
      → **2026-07-06 三好さんの導出でほぼ解決（有力仮説）**。詳細下記「N形2次元マップの
      再構成方法」参照。専用の2次元スキャンスクリプトは不要で、`step2_para.py`が出す
      1次元の Et(z)・Ep(z) カーブの値を組み合わせるだけで任意のθ_incl・φ_incl点
      （N形含む）のEintra(6)を再構成できることを線形代数で検証済み。
      **ただし「大野さんが実際にこの方法で計算したか」はまだ未確認**（要確認のまま）:
      「論文Fig.5bのθ_incl・φ_incl 2次元マップ（N形の発見）はこの方法で計算しましたか？」

### N形2次元マップの再構成方法（2026-07-06、三好さんの導出・検証済み）
...
- [ ] **新規（2026-07-06）**: `legacy/ono_scripts/XRD_pattern/`（論文§2.7、粉末XRD比較）を
      csp本体・GUIに統合するか三好さんに確認（tot_energy/と違いCLAUDE.mdのスコープ外
      リストには入っていないので、統合前提で考えてよいかもしれない）
