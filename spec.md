...
- **φ_inclの扱いに注意**: 論文ではφ_incl=0°/90°でグライド対称保持（→G形）、中間値で
  ...
  N形（中間的なφ_incl）を探すにはT型・SP型で別々のシフト量が必要なはずで、これは
  `step3_para.py`が持つ**`Rt`（T型用）と`Rp`（SP型用）という独立変数に対応している可能性が高い**
  （未確認の仮説。次に大野さんと話す際に確認するとよい、残タスクに追加済み）
...
- [ ] **新規**: `step3_para.py` の `Rt`（T型用）・`Rp`（SP型用）が、論文§2.2のφ_incl中間値
      （N形、グライド対称が破れるケース）に対応するという仮説を大野さんに確認する
