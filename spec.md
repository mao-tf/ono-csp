...
> 対応論文: Ohno et al., "Origin of Layered Herringbone Packing and Polymorphism in Polyacenes: A Quantum Chemical Optimization Approach", JACS (submitted)
...
- X軸（面法線）: 両方0 → **完全一致**
- Y軸（短軸）: 符号含め完全一致
- Z軸（長軸）: **符号が反転**（例: anthracene で Ohno側 Z=-1.221 vs csp側 Z=+1.221）
...
4. **`tcal_csv.py` の `/path/to/tcal_csv/{work_dir}` という無意味な固定プレフィックス**
   ...
   `--auto-dir` オプションが事実上機能していなかった（Ohno さんのローカル環境の絶対パスが
...
5. **2層構造が重なって見える不具合（2026-07-06、三好さん報告・重大なバグ）**: 実際に
   ...
   **Rt・Rpを0以外にすると実際に原子が重なっていた**（例: Ra=2.4,Rb=-1.1で
   ...
   Ohno さんの元コード自身もこの`z_shift`抜きの値を返す設計で、csp側の数値検証はこれと
...
### GUI用語の統一・不明瞭な表現の修正（2026-07-07、三好さん指摘）

1. **「SP contacts」は論文にもOnoさんのコードにも登場しない略語**と判明
   （論文は常に"slipped-parallel"とフルスペル、大野さんのコードのコメントも同様）。
   `app.py`のユーザー向けテキスト内の"SP"表記を全て"slipped-parallel"に統一。
   なお`src/csp/`内のコード側docstring・コメントには開発者向けとして"SP"の略記が
   まだ残っている（ユーザーが直接見る部分ではないため今回は対象外）
2. **「same algorithm as legacy step1.py --init」という表現が誤解を招く**と指摘。
   Tab2・Tab4のvdW事前スキャンの説明文で、実際に動いているのは大野さんの元スクリプトの
   呼び出しではなく**こちらが新規に書いたベクトル化版**（`step1a_scan()`,
   `interlayer_vdw_scan()`）なのに、「legacy step1.py」という書き方だと元スクリプトを
   直接実行しているように読めてしまう。「元スクリプトを直接呼んでいるのではなく、
   その出力を数値的に再現することを検証済みの、GUI内で直接動く高速なベクトル化再実装」
   と明記するよう修正
