...
| ファイル | 列 | 対応タブ | 状態 |
|---------|-----|---------|------|
| `step1_init_params.csv` | a, b, theta, status | Tab 2（vdW 初期候補） | **生成済み（2026-07-06。大野さんから受領していないが、`step1.py`の`init_process()`をペンタセンでローカル実行するだけで再現可能。DFT不要・数秒で終わる。受領済みのstep1.csvと突合し22候補全て一致することを検証済み）** |
